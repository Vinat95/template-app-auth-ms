...
let AppModule = class AppModule {
    configure(consumer) {
        consumer
            .apply(response_middleware_1.ResponseMiddleware)
...
exports.AppModule = AppModule = __decorate([
    (0, common_1.Module)({
        ...
        providers: [app_service_1.AppService, response_service_1.ResponseService, response_interceptor_1.ResponseInterceptor, jwt_strategy_1.JwtStrategy],
    })
], AppModule);
...
let ResponseInterceptor = class ResponseInterceptor {
    ...
    intercept(context, next) {
        ...
        }), (0, operators_1.catchError)((error) => (0, rxjs_1.throwError)(() => this.handleError(error))));
    }
    handleError(error) {
        if (error instanceof common_1.HttpException) {
            const statusCode = error.getStatus();
            return new common_1.HttpException(this.responseService.error(statusCode, error.message), statusCode);
        }
        else if (error instanceof Error) {
            return new common_1.HttpException(this.responseService.error(common_1.HttpStatus.INTERNAL_SERVER_ERROR, 'Errore interno del server'), common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
};
...
exports.ResponseInterceptor = ResponseInterceptor = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeof (_a = typeof response_service_1.ResponseService !== "undefined" && response_service_1.ResponseService) === "function" ? _a : Object])
], ResponseInterceptor);
...
let ResponseService = class ResponseService {
    success(data, message = 'Operazione completata con successo', statusCode = 200) {
        return {
            success: true,
            data,
            message,
...
let ResponseMiddleware = class ResponseMiddleware {
    ...
    use(req, res, next) {
        res['success'] = (data, message = 'Operazione completata con successo', statusCode = 200) => {
            res
                .status(statusCode)
